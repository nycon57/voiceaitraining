...
function ButtonLoadingDemo() {
  ...
  return (
    <div className="space-y-4">
      <Button onClick={showSaveLoading} disabled={isLoading} className="gap-2">
...
function ContentLoadingDemo() {
  const [isLoading, startContentTransition] = useTransition()
...
function ProgressLoadingDemo() {
  const [{ progress, isRunning }, dispatch] = useReducer(
    (state: { progress: number; isRunning: boolean }, action: { type: 'start' | 'reset' | 'tick' }) => {
      switch (action.type) {
        case 'start':
          return { progress: 0, isRunning: true }
        case 'reset':
          return { progress: 0, isRunning: false }
        case 'tick': {
          const nextProgress = Math.min(100, state.progress + Math.random() * 15)
          return { progress: nextProgress, isRunning: nextProgress < 100 }
        }
      }
    },
    { progress: 0, isRunning: false },
  )
    ...
    <div className="space-y-4">
      ...
      {showSuccess && (
        <div className="flex items-center gap-2 text-green-600">
          <CheckCircle className="size-5 animate-in fade-in zoom-in duration-500" />
...
export function InteractionDesignSection() {
  ...
  return (
    <div className="space-y-12">
      {/* Header */}
      <div className="space-y-4">
        <h1 className="text-4xl font-semibold tracking-tight">Interaction Design</h1>
      ...
      {/* Component States */}
      <InteractionDesignStatesSection />

      <Separator />

      {/* Loading States */}
      <InteractionDesignLoadingSection />

      <Separator />

      {/* Error States */}
      <InteractionDesignErrorsSection />

      <Separator />

      {/* Micro-interactions */}
      <InteractionDesignMicroSection />
    </div>
  )
}

function InteractionDesignStatesSection() {
  return (
          <section id="states" className="space-y-8">
            <div className="space-y-4">
              <h2 className="text-3xl font-semibold">Component States</h2>
...
function InteractionDesignLoadingSection() {
  return (
...
function InteractionDesignErrorsSection() {
  return (
          <section id="errors" className="space-y-8">
            <div className="space-y-4">
              <h2 className="text-3xl font-semibold">Error States</h2>
              <p className="text-lg text-muted-foreground">
            ...
            <div className="space-y-4">
              {errorStates.map((error, index) => (
                <ErrorStateDemo key={JSON.stringify(error)} error={error} />
              ))}
            </div>
...
function InteractionDesignMicroSection() {
  return (
          <section id="micro" className="space-y-8">
            <div className="space-y-4">
              <h2 className="text-3xl font-semibold">Micro-interactions</h2>
              <p className="text-lg text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="flex items-center gap-3">
                    <div className="size-3 bg-blue-500 rounded-full animate-pulse"></div>
