...
const emptyPreviousAttempts: NonNullable<CallAnalysisScreenProps['previousAttempts']> = []
...
export function CallAnalysisScreen({
  ...
  const sectionProps = {
    scenario, kpis, transcript, analysis, streamedText, isAnalyzing, attemptId,
    attemptNumber, previousAttempts, averageScore, bestScore, onRetry, scoreBadge,
    currentAttemptStats, handleDownloadTranscript, handleDownloadReport,
  }
  ...
  return (
    <div className="container max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center space-y-4 mb-10">
        <div className="inline-flex items-center justify-center size-20 rounded-full bg-primary/10 mb-2 animate-in zoom-in duration-500">
      ...
      {/* Score Card */}
      {analysis && (
        <CallAnalysisSection1 {...sectionProps} />
      )}

      {/* Score Breakdown */}
      {analysis && analysis.scoreBreakdown && (
        <CallAnalysisSection2 {...sectionProps} />
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-2 mb-8">
        ...
        {/* Performance Metrics */}
        <CallAnalysisSection3 {...sectionProps} />
      </div>
      ...
      {analysis && analysis.nextSteps && analysis.nextSteps.length > 0 && (
        <CallAnalysisSection4 {...sectionProps} />
      )}
      ...
      {/* Export Actions */}
      {attemptId && (
        <CallAnalysisSection5 {...sectionProps} />
      )}
...
type CallAnalysisSectionProps = {
  scenario: CallAnalysisScreenProps['scenario']
  kpis: CallAnalysisScreenProps['kpis']
  transcript: CallAnalysisScreenProps['transcript']
  analysis: any
  streamedText: string
  isAnalyzing: boolean
  attemptId?: string
  attemptNumber: number
  previousAttempts: NonNullable<CallAnalysisScreenProps['previousAttempts']>
  averageScore?: number
  bestScore?: number
  onRetry?: () => void
  scoreBadge: ReturnType<typeof getScoreBadge> | null
  currentAttemptStats: any
  handleDownloadTranscript: () => Promise<void>
  handleDownloadReport: () => Promise<void>
}

function CallAnalysisSection1(props: CallAnalysisSectionProps) {
  const { scenario, kpis, transcript, analysis, streamedText, isAnalyzing, attemptId, attemptNumber, previousAttempts, averageScore, bestScore, onRetry, scoreBadge, currentAttemptStats, handleDownloadTranscript, handleDownloadReport } = props
  return (
...
function CallAnalysisSection2(props: CallAnalysisSectionProps) {
  const { scenario, kpis, transcript, analysis, streamedText, isAnalyzing, attemptId, attemptNumber, previousAttempts, averageScore, bestScore, onRetry, scoreBadge, currentAttemptStats, handleDownloadTranscript, handleDownloadReport } = props
  return (
            <Card className="mb-8">
              <CardHeader>
              ...
              <CardContent className="space-y-5">
                {analysis.scoreBreakdown.map((item: any, index: number) => (
...
function CallAnalysisSection3(props: CallAnalysisSectionProps) {
  const { scenario, kpis, transcript, analysis, streamedText, isAnalyzing, attemptId, attemptNumber, previousAttempts, averageScore, bestScore, onRetry, scoreBadge, currentAttemptStats, handleDownloadTranscript, handleDownloadReport } = props
  return (
            <Card>
              <CardHeader>
...
function CallAnalysisSection4(props: CallAnalysisSectionProps) {
  const { scenario, kpis, transcript, analysis, streamedText, isAnalyzing, attemptId, attemptNumber, previousAttempts, averageScore, bestScore, onRetry, scoreBadge, currentAttemptStats, handleDownloadTranscript, handleDownloadReport } = props
  return (
              ...
              <CardContent>
                <ul className="space-y-3">
                  {analysis.nextSteps.map((step: string, index: number) => (
                    <li key={JSON.stringify(step)} className="flex items-start gap-3 text-sm">
                      <Badge
                        variant="outline"
...
function CallAnalysisSection5(props: CallAnalysisSectionProps) {
  const { scenario, kpis, transcript, analysis, streamedText, isAnalyzing, attemptId, attemptNumber, previousAttempts, averageScore, bestScore, onRetry, scoreBadge, currentAttemptStats, handleDownloadTranscript, handleDownloadReport } = props
  return (
