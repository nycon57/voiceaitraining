...
export default function LeaderboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const mostImproved = mockLeaderboard.reduce((best, entry) =>
      entry.trend > best.trend ? entry : best
    )
    ...
    <>
      ...
      <div className="space-y-6 p-4">
        ...
        <div className="flex flex-col gap-2">
          <h1 className="font-headline text-3xl font-semibold tracking-tight">Leaderboard</h1>
        ...
        <LeaderboardRankingsTable entries={mockLeaderboard.slice(3)} />
...
function CurrentUserRankCard({ currentUser }: { currentUser: LeaderboardEntry }) {
  return (
        ...
        <div className="grid gap-6 md:grid-cols-[auto_1fr_auto]">
          <div className="flex items-center justify-center">
            <div className="relative">
              <div className="flex size-20 items-center justify-center rounded-full bg-gradient-to-br from-chart-1 via-chart-2 to-chart-3 p-1">
              ...
              {currentUser.trend > 0 && (
                <div className="absolute -right-1 -top-1 flex size-6 items-center justify-center rounded-full bg-success text-white">
          ...
          <div className="flex flex-col justify-center gap-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-headline text-xl font-semibold">{currentUser.name}</h3>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-1 md:gap-3">
            <RankStat label="Score" value={currentUser.score} />
            <RankStat label="Attempts" value={currentUser.attempts} />
            <div className="col-span-2 rounded-lg border bg-muted/50 p-3 text-center md:col-span-1">
              ...
              <div className="flex items-center justify-center gap-1">
                {currentUser.trend > 0 && <TrendingUp className="size-4 text-success" />}
                {currentUser.trend < 0 && <TrendingDown className="size-4 text-destructive" />}
                {currentUser.trend === 0 && <Minus className="size-4 text-muted-foreground" />}
                <p className={cn("font-headline text-xl font-bold", currentUser.trend > 0 && "text-success", currentUser.trend < 0 && "text-destructive")}>
...
function LeaderboardRankingsTable({ entries }: { entries: LeaderboardEntry[] }) {
  return (
    <Card animated={false}>
      <CardHeader>
...
function LeaderboardRow({ entry }: { entry: LeaderboardEntry }) {
  return (
    <TableRow className={cn(entry.isCurrentUser && "bg-primary/5 hover:bg-primary/10")}>
      <TableCell>
        <span className={cn("font-headline text-lg font-bold", getRankColor(entry.rank))}>
          #{entry.rank}
        </span>
      </TableCell>
      <TableCell>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {entry.isCurrentUser && <Badge variant="brand" size="sm">You</Badge>}
            </div>
      ...
      <TableCell className="text-center hidden md:table-cell">
        <div className="flex items-center justify-center gap-1">
          {entry.trend > 0 && <TrendingUp className="size-4 text-success" />}
          {entry.trend < 0 && <TrendingDown className="size-4 text-destructive" />}
          {entry.trend === 0 && <Minus className="size-4 text-muted-foreground" />}
          <span className={cn("font-semibold", entry.trend > 0 && "text-success", entry.trend < 0 && "text-destructive", entry.trend === 0 && "text-muted-foreground")}>
      ...
      <CardContent>
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <LeaderboardMetric icon={Users} label="Participants" value={stats.totalParticipants} />
            <LeaderboardMetric icon={Target} label="Avg Score" value={stats.avgScore} />
          </div>
          <div className="rounded-lg border bg-gradient-to-br from-warning/10 to-warning/5 p-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">{topPerformer.name}</p>
              </div>
              <Crown className="size-12 text-warning/30" />
            </div>
          </div>
          <MostImprovedCard entry={stats.mostImproved} />
...
function LeaderboardMetric({ icon: Icon, label, value }: { icon: ElementType; label: string; value: number }) {
  return (
...
function MostImprovedCard({ entry }: { entry: LeaderboardEntry }) {
  return (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-1 mt-1">
            ...
            <span className="text-lg font-bold text-success">+{entry.trend}</span>
          </div>
        </div>
        <Avatar size="xl">
          <AvatarImage src={entry.avatar} />
      ...
      <CardContent className="pt-6 text-center">
        ...
        <div className="flex justify-center mb-4">
          <div className={cn("rounded-full p-4", config.bg)}>
            <MedalIcon className={cn("size-8", config.color)} />
          </div>
        </div>
